...
2. **Find the current work context.** Locate the active sprint / milestone / board your project uses, and see where your ticket fits. Its `## Objective` is the sprint's ROOT: one problem-space sentence saying what problem the work serves. **Your assignment also quotes it verbatim** — the one thing besides your assignment that a brief carries — so you hold it even if your worktree's copy of the plan is older than the orchestrator's. If the two differ, the brief's is current; say so in your report. **The ticket tells you what to build; the objective tells you what it is for. When the two come apart — the ticket as written could be completed in full without moving the objective — say that in your report instead of quietly building the ticket.** That is not scope refusal; it is the one observation nobody downstream can make for you.
...
- **Report your status update — do not write it yourself.** You cannot persist a planning/tracking doc from a worktree: the worktree copy is blocked by `protect-worktree-planning.sh`, and the platform's built-in shared-checkout guard blocks the primary copy. Do not fight either guard. Put the full status update in your final report to the orchestrator — what you built, key decisions made, test results, and any issues or follow-ups — and the orchestrator writes it to your project's status-update location. Do NOT write any planning/tracking docs yourself; the orchestrator owns all planning persistence.
